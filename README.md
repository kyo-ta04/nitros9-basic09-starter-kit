...
| ディレクトリ名 | 役割 |
| :--- | :--- |
| **`exec09`** | exec09-1、Arto Salmi氏の6809シミュレータを拡張したCPUシミュレータ（`m6809-run`）。 |
...
### 3. NitrOS-9/BASIC09の起動

**OS-9起動時の日付入力に不具合があり、最初に文字入力すると .......... が入るので [BS]で削除してから現在時刻を入力します。**
...
#### 終了
CTRL-Cを数回押してから[ENTER]でデバッガに戻るので、 q コマンドで終了します。
...
3. **BASIC09用ディスクイメージの作成・再構築をする場合**
   ```bash
   cd nitros9-runtime
