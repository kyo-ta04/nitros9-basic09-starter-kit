...
Init                clrb                          default to no error...
                  ...
                  IFNE    mc09
* Multicomp09 / exec09: polled ACIA only (no Rx/Tx IRQ).
* Banner and shell I/O work by polling Stat.TxE / Stat.RxF.
* Skipping F$IRQ avoids a fragile UART-IRQ path that hangs Open/Write.
                    ifne      H6309
...
NoDTR               ldx       <V.PORT             get port address
                  ...
                  IFNE    mc09
* 6850 polled: master reset ($03), then $15 =
*   div16, 8N1, RTS low, Tx IRQ off, Rx IRQ off
                    lda       #$03
                    sta       CtlReg,x
                    lda       #$15
                    sta       CtlReg,x
                    clr       <Cpy.Stat
                    clr       <Mask.DCD            no DCD/DSR flow control
                    clr       <Mask.DSR
                    clr       <Wrk.Type            no software/hardware handshake
                    puls      cc,dp,pc
                  ELSE
...
ChkState            equ       *
                    ...
                    bitb      #Condem
                    endc
                    lbne      PrAbtErr            yes, go do it... (long: polled Rx grew module)
...
ReadChar            ldb       <V.ERR              get accumulated errors, if any
                  ...
                  IFNE    mc09
* Polled Rx: no UART IRQ. If the software buffer is empty, wait for
* Stat.RxF, sleeping one tick at a time so the 50Hz clock keeps running.
                    bne       GotData
PollRx              andcc     #^IntMasks          allow timer IRQ while waiting
                    ldx       <V.PORT
                    lda       StatReg,x
                    bita      #Stat.RxF           character available?
                    bne       TakeRx
                    lbsr      Sleep1              yield until next tick
                    bra       PollRx
TakeRx              orcc      #IntMasks
                    lda       DataReg,x           read data register
                    puls      cc,dp,pc
GotData             equ       *
                  ELSE
                  ...
                  ENDC
                    ifne      H6309
                    decd                          character we're about to grab
                    else
                    subd      #$0001
                    endc
